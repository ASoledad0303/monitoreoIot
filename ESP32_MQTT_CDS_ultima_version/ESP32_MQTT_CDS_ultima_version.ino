...
// ======== Ajuste de offset en reposo =======
float I_OFFSET_CORRECTION = 2.0f;  // Ajustar este valor para que en reposo sea 0A
...
#define MQTT_HOST "138.197.237.23"
//#define MQTT_HOST   "192.168.100.64"
//#define MQTT_HOST   "10.143.221.168" // IP del broker MQTT
#define MQTT_PORT   1883
#define MQTT_USER   "tesis"               // si broker no requiere, dejar vacío
#define MQTT_PASS   "sE2wBB29123w"               // idem
...
void sampleWindow(float off_ct, float off_v,
  ...
  while ((millis()-t0) < WINDOW_MS) {
    ...
    // ========== CORRECCIÓN DE OFFSET EN REPOSO ==========
    // Aplicar corrección para que en reposo sea 0A
    if (fabs(i_inst) < 3.0f) { // Si la corriente es muy baja (reposo)
      if (i_inst >= 0) {
        i_inst = max(0.0f, i_inst - I_OFFSET_CORRECTION);
      } else {
        i_inst = min(0.0f, i_inst + I_OFFSET_CORRECTION);
      }
    }
    
    // ========== CONDICIÓN DE AJUSTE PARA SCT-013-000 ==========
    // Aplicar corrección basada en el valor de corriente instantánea
    else if (fabs(i_inst) < 10.0f) {
      // Si la corriente es menor a 10A, restar 0.992A
      if (i_inst >= 0) {
        i_inst = max(0.0f, i_inst - 0.992f);
      } else {
        i_inst = min(0.0f, i_inst + 0.992f);
      }
    } else if (fabs(i_inst) > 23.0f) {
      // Si la corriente supera los 23A, sumar 2.5A
      if (i_inst >= 0) {
        i_inst += 2.5f;
      } else {
        i_inst -= 2.5f;
...
void saveCal() {
  ...
  prefs.putFloat("I_OFFSET", I_OFFSET_CORRECTION);
...
void loadCal() {
  ...
  I_OFFSET_CORRECTION = prefs.getFloat("I_OFFSET", 2.0f);
  stage      = RUN; // Siempre stage 4
  prefs.end();
  Serial.printf("[NVS] Loaded V=%.2f I=%.3f PH=%.3f I_POL=%.0f I_OFFSET=%.2f ST=4 (RUN)\n",
                V_CAL_GAIN, I_CAL_GAIN, PHASE_CAL, I_POL, I_OFFSET_CORRECTION);
}
...
void printHelp() {
  ...
  Serial.println(F("ioffset <v>    -> set I_OFFSET (default: 2.0)"));
...
void loop() {
  ...
  while (Serial.available()) {
    ...
    if (c=='\r' || c=='\n') {
      ...
      if (rx.length()) {
        ...
        } else if (rx.startsWith("ioffset")) {
          float v = rx.substring(7).toFloat();
          I_OFFSET_CORRECTION = v;
          Serial.printf("[OK] I_OFFSET=%.2f\n", v);
  ...
  Serial.printf("RUN  V=%.1f I=%.3f  P=%.1fW(sig)  S=%.1fVA  PF=%.3f(sig)  I_POL=%.0f  I_OFFSET=%.2f\n",
                Vrms, Irms, P, S, PF, I_POL, I_OFFSET_CORRECTION);
